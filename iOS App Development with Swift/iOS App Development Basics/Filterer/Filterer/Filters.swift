...
class ImageFilterManager: ImageFilter {
    let predefinedFilters: [String:ImageFilter] = ["150% brightness": BrightnessFilter(brightness: 1.5),
                                                   "red": ColorBoostingFilter(color: Color.RED, boost: 10),
                                                   "blue": ColorBoostingFilter(color: Color.BLUE, boost: 10),
                                                   "green": ColorBoostingFilter(color: Color.GREEN, boost: 10),
                                                   "sharper": SharpeningFilter()]
    
    var filters = [ImageFilter]()
    
    func addFilter(filter: String) {
        filters.append(predefinedFilters[filter.lowercaseString]!)
    }
